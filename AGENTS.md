...
- **Layer packages:** `providers/` (LLM abstraction), `tools/` (one file per tool behind a Protocol + registry, plus shared `arguments.py` parsing helpers), `agent/` (orchestration loop, event-log state, prompt assembly), `commands/` (registry + one file per `/`-prefixed REPL directive), `ui/` (rich-rendered output + prompt_toolkit input, including reusable interaction prompts).
